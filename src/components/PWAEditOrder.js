...
const PWAEditOrder = () => {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      const orderResponse = await authenticatedFetch(`${API_ENDPOINTS.ORDERS}${orderNumber}/`);
      if (orderResponse.ok) {
        ...
        setDeliveryType(orderData.delivery_type || 'Pickup');
        ...
        if (orderData.items && orderData.items.length > 0) {
          orderData.items.forEach(item => {
            ...
            if (menuItemId) {
              // Check if this item is an extra or main item
              // We'll determine this after menu items are loaded
              itemsMap[menuItemId] = item.quantity;
            }
          });
        }
  ...
  // Re-categorize items when menu items are loaded
  useEffect(() => {
    if (allMenuItems.length > 0 && originalOrder && originalOrder.items) {
      const itemsMap = {};
      const extrasMap = {};
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      if (!token) {
        toast.error('Please log in to edit an order.');
        navigate('/login');
        return;
      }
      ...
      const response = await fetch(`${API_ENDPOINTS.ORDERS}${orderNumber}/`, {
        method: 'PATCH',
        headers: {
      ...
      if (response.ok) {
        ...
        if (response.status === 401) {
          ...
          toast.error(`Failed to update order: ${errorData.detail || errorData.message || 'Unknown error'}`);
        }
      }
    } catch (error) {
      console.error('Network or other error updating order:', error);
      
      if (error.message.includes('Authentication')) {
        toast.error('Authentication failed. Please log in again.');
        navigate('/login');
      } else {
        toast.error('Failed to update order. Please check your connection and try again.');
      }
    } finally {
      setSubmitting(false);
    ...
    return (
      <div className="pwa-content">
        <div className="pwa-loading">
          ...
          <div className="pwa-loading-text">Loading order for editing...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="pwa-content">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <div className="pwa-card-title" style={{ margin: 0, fontSize: '1.1rem' }}>
              Edit Order #{originalOrder.order_number || originalOrder.id}
            </div>
        ...
        <div className="pwa-card">
          ...
          <div className="pwa-list">
            <div className="pwa-list-item">
              <div className="pwa-list-content">
                <div className="pwa-list-title">Subtotal</div>
              </div>
          ...
          >
            {submitting ? (
              ...
              <>
                ...
                Update Order ({formatPrice(grandTotal)})
              </>
            )}
          </button>
        </div>
              ...
              <div className="pwa-list" style={{ maxHeight: '300px', overflow: 'auto' }}>
                {filteredMainItems.map(item => (
                  ...
                  >
                    ...
                    <div className="pwa-list-content">
                      <div className="pwa-list-title">{item.name}</div>
                      <div className="pwa-list-subtitle">{formatPrice(item.price)}</div>
                    </div>
      ...
      {showConfirmModal && (
        <div className="pwa-modal-overlay" onClick={() => setShowConfirmModal(false)}>
          <div className="pwa-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pwa-modal-header">
              <div className="pwa-modal-title">Confirm Order Update</div>
            </div>
            
            <div style={{ padding: '16px 0' }}>
              <p style={{ margin: '0 0 16px 0', fontSize: '0.95rem' }}>Please review your updated order details before confirming:</p>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  <strong style={{ fontSize: '1rem' }}>Order #{originalOrder.order_number || originalOrder.id}</strong>
              ...
              <p style={{ 
                ...
                Click "Confirm" to update this order
              </p>
            </div>
